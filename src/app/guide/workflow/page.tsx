    ...
    <div className="min-h-screen">
      ...
      <div className="w-full sm:max-w-[60%] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        ...
        <section>
          <div className="flex items-center justify-center gap-2 sm:gap-3 flex-wrap">
            {phases.map((p: typeof phases[0], i: number) => {
              ...
              const Icon = phaseIconMap[p.iconName];
              return (
        ...
        {realExamples.map((example) => {
          ...
          const Icon = phaseIconMap[phases.find((p: typeof phases[0]) => p.phase === example.phase)?.iconName || "Cpu"];
          return (
        ...
        <section>
          ...
          <div className="flex flex-col gap-3">
            {phases.map((phase: typeof phases[0], i: number) => {
              const pc = phaseColors[phase.color];
              const Icon = phaseIconMap[phase.iconName];
              return (
                <div key={phase.phase} id={`phase-${phase.phase.toLowerCase()}`} className="flex flex-col sm:flex-row gap-3">
